...
void write_ts_hdf5( const hid_t file_id, int , const qnu* qu, const Lagr* p,
                    const nmpc& C, const float* grad,
                    const unsigned int* sd_loop, const double* sd_loop_time )
{

	// const char *field_names[C.N]  =
	// {
	// 	"x", "y", "Dx", "Dy", "ex", "ey", "th", "Dth", "v",
	// 	"p1", "p2", "P3", "p4", "p5", "grad"
	// };
	...
	// herr_t   status;
	// hid_t dfloat = H5Tcopy( H5T_NATIVE_FLOAT );
	// static hsize_t* dim;
	// *dim = ( int ) C.N;
	// char *grp_name;

	// sprintf( grp_name, "/%d", C.horizon_loop );

	// grp = H5Gcreate2( file_id, grp_name, H5P_DEFAULT, H5P_DEFAULT,
	//                   H5P_DEFAULT );
	// dataspace = H5Screate_simple( 1, dim, NULL );

	// dataset = H5Dcreate2( grp, "x", dfloat, dataspace,
