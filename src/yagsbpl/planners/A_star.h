...
public:
	...
	A_star_planner()
	{
		...
		event_NodeExpanded_g = NULL;
		event_NodeExpanded_nm = NULL;
		event_SuccUpdated_g = NULL;
		event_SuccUpdated_nm = NULL;
		heapFun_fp = NULL;
	}
	void setParams( double eps = 1.0 , int heapKeyCt = 20 , int progressDispInterval = 10000 ) // call to this is optional.
	{
		subopEps = eps;
		heapKeyCount = heapKeyCt;
		ProgressShowInterval = progressDispInterval;
	}
	...
	{
		init( &theEnv );   // for version compatability
	}
	void plan( void );
