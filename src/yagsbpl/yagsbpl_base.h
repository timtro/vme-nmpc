...
bool YAGSBPL_vDisplay_done = true;
...
public:
	...
	int size( void )
	{
		if ( targets ) return targets->size();
		else return -1;
	}
...
public:
	...
	{
		if( p && getHashBin_fp ) return ( ( p->*getHashBin_fp )( n ) );
		else this->func_redefined = false;
	}
	bool isAccessible( NodeType& n )
	{
		if( p && isAccessible_fp ) return ( ( p->*isAccessible_fp )( n ) );
		else this->func_redefined = false;
	}
	void getSuccessors( NodeType& n, std::vector<NodeType>* s, std::vector<CostType>* c )
	{
		if( p && getSuccessors_fp ) return ( ( p->*getSuccessors_fp )( n, s, c ) );
		else this->func_redefined = false;
	}
	void getPredecessors( NodeType& n, std::vector<NodeType>* s, std::vector<CostType>* c )
	{
		if( p && getPredecessors_fp ) return ( ( p->*getPredecessors_fp )( n, s, c ) );
		else this->func_redefined = false;
	}
	CostType getHeuristics( NodeType& n1, NodeType& n2 )
	{
		if( p && getHeuristics_fp ) return ( ( p->*getHeuristics_fp )( n1, n2 ) );
		else this->func_redefined = false;
	}
	bool storePath( NodeType& n )
	{
		if( p && storePath_fp ) return ( ( p->*storePath_fp )( n ) );
		else this->func_redefined = false;
	}
	bool stopSearch( NodeType& n )
	{
		if( p && stopSearch_fp ) return ( ( p->*stopSearch_fp )( n ) );
		else this->func_redefined = false;
	}
};
