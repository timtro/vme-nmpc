...
struct ITestCaseRegistry {
  ...
  virtual void getFilteredTests( TestSpec const& testSpec, IConfig const& config,
                                 std::vector<TestCase>& matchingTestCases, bool negated = false ) const = 0;
...
inline ResultDisposition::Flags operator | ( ResultDisposition::Flags lhs,
  ...
  return static_cast<ResultDisposition::Flags>( static_cast<int>
...
class ValuesGenerator : public IGenerator<T> {
  ...
  operator T () const {
    size_t overallIndex = getCurrentContext().getGeneratorIndex( m_fileInfo,
        m_totalSize );
...
namespace Catch {
  ...
  friend bool operator == ( double lhs, Approx const& rhs ) {
    ...
    return fabs( lhs - rhs.m_value ) < rhs.m_epsilon * (rhs.m_scale + (std::max)(
...
namespace Catch {
  ...
  virtual Ptr<Matcher<ExpressionT> > clone() const {
    return Ptr<Matcher<ExpressionT> >( new DerivedT( static_cast<DerivedT const&>
                                       ( *this ) ) );
  }
                                   };
  ...
  template<typename ExpressionT>
  inline Impl::Generic::AllOf<ExpressionT> AllOf( Impl::Matcher<ExpressionT>
      const& m1,
  ...
  template<typename ExpressionT>
  inline Impl::Generic::AnyOf<ExpressionT> AnyOf( Impl::Matcher<ExpressionT>
      const& m1,
      Impl::Matcher<ExpressionT> const& m2 ) {
...
namespace Catch {
  ...
  struct TagAlias {
    TagAlias( std::string _tag, SourceLineInfo _lineInfo ) : tag( _tag ),
  lineInfo( _lineInfo ) {}
...
inline size_t registerTestMethods() {
  ...
  Class* classes = (CATCH_UNSAFE_UNRETAINED Class*)malloc( sizeof(
        Class) * noClasses);
    ...
    Class cls = classes[c];
    {
      ...
      for ( u_int m = 0; m < count ; m++ ) {
        ...
        if ( startsWith( methodName, "Catch_TestCase_" ) ) {
          ...
          getMutableRegistryHub().registerTest( makeTestCase( new OcMethod( cls,
                                                selector ), className, name.c_str(), desc.c_str(), SourceLineInfo() ) );
          noTestMethods++;
        }
      }
...
public:
NamePattern( std::string const& name ) : m_name( toLower( name ) ),
...
bool matches( TestCaseInfo const& testCase ) const {
  ...
  for ( std::vector<Ptr<Pattern> >::const_iterator it = m_patterns.begin(),
        itEnd = m_patterns.end(); it != itEnd; ++it )
...
bool matches( TestCaseInfo const& testCase ) const {
  ...
  for ( std::vector<Filter>::const_iterator it = m_filters.begin(),
        itEnd = m_filters.end(); it != itEnd; ++it )
    if ( it->matches( testCase ) )
      return true;
...
namespace Catch {
  ...
  class TestSpecParser {
    ...
    public:
    TestSpecParser( ITagAliasRegistry const& tagAliases ) : m_tagAliases(
&tagAliases ) {}
...
BoundArgFunction( IArgFunction<ConfigT>* _functionObj ) : functionObj(
  _functionObj ) {}
BoundArgFunction( BoundArgFunction const& other ) : functionObj(
  other.functionObj ? other.functionObj->clone() : NULL ) {}
BoundArgFunction& operator = ( BoundArgFunction const& other ) {
  IArgFunction<ConfigT>* newFunctionObj = other.functionObj ?
...
bool hasShortName( std::string const& shortName ) const {
  return std::find( shortNames.begin(), shortNames.end(),
                    shortName ) != shortNames.end();
}
...
class CommandLine {
  ...
  struct Arg : CommonArgProperties<ConfigT>, OptionArgProperties,
...
std::vector<Parser::Token> populateOptions( std::vector<Parser::Token> const&
    tokens, ConfigT& config ) const {
...
std::vector<Parser::Token> populateFixedArgs( std::vector<Parser::Token> const&
    tokens, ConfigT& config ) const {
  std::vector<Parser::Token> unusedTokens;
  int position = 1;
  ...
  for ( std::size_t i = 0; i < tokens.size(); ++i ) {
    Parser::Token const& token = tokens[i];
    typename std::map<int, Arg>::const_iterator it = m_positionalArgs.find(
      position );
...
std::vector<Parser::Token> populateFloatingArgs( std::vector<Parser::Token>
    const& tokens, ConfigT& config ) const {
  if ( !m_floatingArg.get() )
    return tokens;
                     ...
                     Catch::cout() << Text( testCaseInfo.tagsAsString, tagsAttr ) << std::endl;
                                ...
                                if ( !config.testSpec().hasFilters() )
                                  Catch::cout() << pluralise( matchedTests, "test case" ) << "\n" << std::endl;
                                      ...
                                      Catch::cout() << pluralise( matchedTests,
                                          "matching test case" ) << "\n" << std::endl;
...
inline std::size_t listTags( Config const& config ) {
  ...
  for ( std::vector<TestCase>::const_iterator it = matchedTestCases.begin(),
        ...
        it != itEnd;
        ++it ) {
    for ( std::set<std::string>::const_iterator  tagIt =
          it->getTestCaseInfo().tags.begin(),
          tagItEnd = it->getTestCaseInfo().tags.end();
          tagIt != tagItEnd;
          ++tagIt ) {
      std::string tagName = *tagIt;
      std::string lcaseTagName = toLower( tagName );
      std::map<std::string, TagInfo>::iterator countIt = tagCounts.find(
        lcaseTagName );
  ...
  for ( std::map<std::string, TagInfo>::const_iterator countIt =
        tagCounts.begin(),
...
inline TrackedSection* TrackedSection::findChild( std::string const&
    childName ) {
...
inline TrackedSection* TrackedSection::acquireChild( std::string const&
    childName ) {
  if ( TrackedSection* child = findChild( childName ) )
    return child;

  m_children.insert( std::make_pair( childName, TrackedSection( childName,
                                     this ) ) );
...
void testGroupEnded( std::string const& testSpec, Totals const& totals,
  ...
  m_reporter->testGroupEnded( TestGroupStats( GroupInfo( testSpec, groupIndex,
                              groupsCount ), totals, aborting() ) );
}
...
virtual void assertionEnded( AssertionResult const& result ) {
  ...
  if ( m_reporter->assertionEnded( AssertionStats( result, m_messages,
                                   m_totals ) ) )
    m_messages.clear();
...
virtual void sectionEnded( SectionInfo const& info,
                           Counts const& prevAssertions, double _durationInSeconds ) {
  ...
  m_reporter->sectionEnded( SectionStats( info, assertions, _durationInSeconds,
                                          missingAssertions ) );
  m_messages.clear();
}
...
bool aborting() const {
  return m_totals.assertions.failed == static_cast<std::size_t>
  ( m_config->abortAfter() );
}
...
void handleUnfinishedSections() {
  ...
  for ( std::vector<UnfinishedSections>::const_reverse_iterator it =
...
virtual void getFilteredTests( TestSpec const& testSpec, IConfig const& config,
                               std::vector<TestCase>& matchingTestCases, bool negated = false ) const {

  for ( std::vector<TestCase>::const_iterator  it = m_functionsInOrder.begin(),
        itEnd = m_functionsInOrder.end();
        it != itEnd;
        ++it ) {
    bool includeTest = testSpec.matches( *it ) && ( config.allowThrows()
        || !it->throws() );
...
inline std::string extractClassName( std::string const&
                                     classOrQualifiedMethodName ) {
...
namespace Catch {
  ...
  NotImplementedException::NotImplementedException( SourceLineInfo const&
      lineInfo )
:   m_lineInfo( lineInfo ) {
  std::ostringstream oss;
...
Stream createStream( std::string const& streamName ) {
  ...
  if ( streamName == "debug" ) return Stream( new
        StreamBufImpl<OutputDebugWriter>, true );
...
namespace Catch {
  ...
  inline TestCaseInfo::SpecialProperties parseSpecialTag( std::string const&
tag ) {
  if ( startsWith( tag, "." ) ||
       tag == "hide" ||
       tag == "!hide" )
...
properties( None ) {
  ...
  for ( std::set<std::string>::const_iterator it = _tags.begin(),
        itEnd = _tags.end(); it != itEnd; ++it ) {
    oss << "[" << *it << "]";
                std::string lcaseTag = toLower( *it );
                properties = static_cast<SpecialProperties>( properties | parseSpecialTag(
                      lcaseTag ) );
...
TestCase::TestCase( ITestCase* testCase,
                    TestCaseInfo const& info ) : TestCaseInfo( info ), test( testCase ) {}
...
namespace Catch {
  LegacyReporterAdapter::LegacyReporterAdapter( Ptr<IReporter> const&
      legacyReporter )
...
bool LegacyReporterAdapter::assertionEnded( AssertionStats const&
    assertionStats ) {
  if ( assertionStats.assertionResult.getResultType() != ResultWas::Ok ) {
    for ( std::vector<MessageInfo>::const_iterator it =
          assertionStats.infoMessages.begin(), itEnd = assertionStats.infoMessages.end();
          it != itEnd;
          ++it ) {
...
void LegacyReporterAdapter::testCaseEnded( TestCaseStats const&
    testCaseStats ) {
...
void LegacyReporterAdapter::testGroupEnded( TestGroupStats const&
    testGroupStats ) {
...
uint64_t getCurrentTicks() {
  timeval t;
  gettimeofday(&t,NULL);
  return static_cast<uint64_t>( t.tv_sec ) * 1000000ull + static_cast<uint64_t>
  ( t.tv_usec );
}
#endif
                       }
...
namespace Catch {
  ...
  void ResultBuilder::useActiveException( ResultDisposition::Flags
                                          resultDisposition ) {
  ...
  std::string ResultBuilder::reconstructExpression() const {
    if ( m_exprComponents.op == "" )
      ...
      m_assertionInfo.macroName + "( " + m_assertionInfo.capturedExpression +
...
class TagAliasRegistry : public ITagAliasRegistry {
    ...
    virtual std::string expandAliases( std::string const& unexpandedTestSpec )
    const;
...
namespace Catch {
  ...
  void TagAliasRegistry::add( char const* alias, char const* tag,
    ...
    if ( !startsWith( alias, "[@" ) || !endsWith( alias, "]" ) ) {
      ...
      oss << "error: tag alias, \"" << alias <<
                                            ...
                                            if ( !m_registry.insert( std::make_pair( alias, TagAlias( tag,
    lineInfo ) ) ).second ) {
      std::ostringstream oss;
  ...
  RegistrarForTagAliases::RegistrarForTagAliases( char const* alias,
      char const* tag, SourceLineInfo const& lineInfo ) {
    try {
...
class XmlReporter : public StreamingReporterBase {
  ...
  virtual void testCaseStarting( TestCaseInfo const& testInfo ) {
    ...
    m_xml.startElement( "TestCase" ).writeAttribute( "name",
        trim( testInfo.name ) );
...
class JunitReporter : public CumulativeReporterBase {
  ...
  void writeGroup( TestGroupNode const& groupNode, double suiteTime ) {
    ...
    xml.scopedElement( "system-out" ).writeText( trim( stdOutForSuite.str() ),
        false );
    xml.scopedElement( "system-err" ).writeText( trim( stdErrForSuite.str() ),
        false );
  }
...
struct ConsoleReporter : StreamingReporterBase {
  ...
  SummaryColumn addRow( std::size_t count ) {
    ...
    for ( std::vector<std::string>::iterator it = rows.begin(); it != rows.end();
          ++it ) {
  ...
  void printTotalsDivider( Totals const& totals ) {
    if ( totals.testCases.total() > 0 ) {
      ...
      stream << Colour( Colour::Error ) << std::string( failedRatio, '=' );
              stream << Colour( Colour::ResultExpectedFailure ) << std::string(
                        failedButOkRatio, '=' );
...
struct CompactReporter : StreamingReporterBase {
  ...
  void printRemainingMessages( Colour::Code colour = dimColour() ) {
    ...
    const std::size_t N = static_cast<std::size_t>( std::distance( itMessage,
        itEnd ) );
