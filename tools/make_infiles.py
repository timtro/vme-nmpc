...
class infile_iterator():
    ...
    def pop(self):
        ...
        fdin.write('N='+'{:d}'.format(self.Nr[self.k])+';\n')
        fdin.write('C='+'{:d}'.format(self.Cr[self.k])+';\n')
        fdin.write('T='+'{:f}'.format(self.Tr[self.k])+';\n')
        fdin.write('eps = 0.08;\n')
        fdin.write('R='+'{:f}'.format(self.Rr[self.k])+';\n')
        fdin.write('Q0='+'{:f}'.format(self.Q0r[self.k])
                +','+'{:d}'.format(self.Q0r[self.k])+';\n')
        fdin.write('Q='+'{:f}'.format(self.Qr[self.k])
                +','+'{:d}'.format(self.Qr[self.k])+';\n')
        fdin.write('m=5;\n')
        fdin.write('n=1;\n')
        fdin.write('dg=0.05;\n')
...
def prep_and_run(infile):
    fdout = open(filename+'.out','w') # what?
...
"""
                        ...
                        subprocess.call([vme_nmpc,'-vf' + filename + '.in'],
                                        stdout = fdout)
                        subprocess.call([analyse,'-f'+filename+'.out','-p'+filename+'.jpg','-csummary.csv'])
                        subprocess.call(['rm',filename + '.in'])
                        subprocess.call(['rm',filename + '.out'])
                        fdout.close()
                        k += 1
